...
const WINDOW_STATE_FILE: &str = "desktop-window.json";
...
pub fn run() -> anyhow::Result<()> {
    ...
    let window_state = load_window_state(&window_state_path);
    let window_tracker = Arc::new(Mutex::new(WindowTracker::new(window_state)));
...
impl WindowTracker {
    ...
    fn attach(&mut self, window: Arc<dioxus::desktop::tao::window::Window>) {
    ...
    fn observe(&mut self, event: &dioxus::desktop::tao::event::Event<'_, impl Sized>, path: &Path) {
    ...
    fn flush(&mut self, window: &dioxus::desktop::tao::window::Window, path: &Path) {
        ...
        match write_window_state(path, state) {
...
/// Replace a private sibling file so a crash leaves the prior record intact.
/// The rename is atomic for readers; directory-entry durability across a
/// machine crash remains best effort because the parent directory is not
/// synchronously flushed.
fn write_window_state(path: &Path, state: WindowState) -> anyhow::Result<()> {
...
mod window_state_tests {
    ...
    fn missing_directory_rejects_write_without_partial_state() {
        ...
        assert!(write_window_state(&path, state).is_err());
        assert!(!path.exists());
        ...
        write_window_state(&path, updated).unwrap();
        assert_eq!(
            parse_window_state(&std::fs::read(&path).unwrap()),
            Some(updated)
        );
