...
pub(crate) struct MenuWiring<A: 'static, Id: 'static, const N: usize> {
    ...
    ///
    /// Written from two directions: synchronously by this row's own focus
    /// moves, and by every item's `onfocusin`/`onfocusout`, which is what
    ...
    /// entirely). That second writer is exactly why a key press does not
    /// read this signal alone to decide where to step from — see
    /// `requested`.
    ...
    /// `begin_open` and dismissal effect) — see each row's own
    /// `menu_requested` for the lifecycle.
    ///
    ...
    /// such second writer — only [`focus_menu_item`] ever sets it — so it
    /// always names the last position keyboard navigation actually asked
    /// for, no matter how late a stray `onfocusin` arrives.
