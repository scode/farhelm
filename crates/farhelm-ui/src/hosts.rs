        ...
        div {
            ...
            div { class: "host-row-main",
                ...
                if confirming_remove {
                    // A stacked, full-width block rather than more
                    // `.host-row-main` flex children on the SAME line as
                    // the name and chip: this state has to fit an
                    // unshrinkable warning sentence, a second copy of the
                    // (unbounded) host name, AND both buttons, which is
                    // more than the 340px sidebar has room for on one
                    // non-wrapping line regardless of how short the name
                    // is — `confirm remove` and `cancel` rendered clipped
                    // and unclickable off the sidebar's edge exactly the
                    // way `remove` itself used to before the row's other
                    // verbs folded into the "⋯" menu (see the section
                    // banner just above, and `.host-confirm-remove-panel`
                    // in app.css for the wrap mechanics). `cancel` is the
                    // safe default here, so keeping it reachable is not a
                    // cosmetic concern.
