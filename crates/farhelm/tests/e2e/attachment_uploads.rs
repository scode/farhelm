...
/// lifecycle claim before the blocking operation returns. Stop must finish
/// while `link` is gated; the late complete attachment then has ordinary
/// retention rather than rollback.
#[farhelm_testtrace::test]
async fn a_publication_that_outlives_its_bound_frees_the_session() {
    ...
    tokio::time::timeout(Duration::from_secs(20), h.client.stop_session(&session.id))
        .await
        .expect("stop must not wait for the gated publication")
        .expect("stop");
