...
/// `--session` flag and its value) while the two read-only verbs need one.
/// A single-verb wrapper would cover only the shorter half and be exactly
/// as long as the slice literal it replaced.
...
/// route answers LIVE from the supervisor, so a row it returns is a session
/// that really exists over there. The listing is served from the helm's
/// cache, where a create that never happened could still appear for one
/// refresh interval if `record_session` seeded it optimistically.
///
...
///
/// SPEC.md's agent section covers a CROSS-host clone, and it is not
/// exercised here: a second host in this suite would have to be an ssh row, which
