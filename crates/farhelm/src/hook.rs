...
//! 4. **No credential, no work.** Without the three injected environment
//!    values there is no supervisor to talk to (someone ran the agent
//!    outside farhelm with our flags somehow present); the run logs
//!    `no-credential` and stops without touching a socket.
