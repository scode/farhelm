...
fn render(agent: &Command) -> String {
    ...
    out.push_str(
        "\n\
         ...
         the hosts listing; create's --profile and clone's own agent are resolved by NAME on\n\
         the target host, and a name that is not there is refused rather than guessed at.\n\
         \n\
...
mod tests {
    ...
    /// The instructions state the two things the creating verbs add that a
    /// model cannot infer from a usage line.
    ///
    /// Both are conventions rather than syntax, which is why the generated
    /// verb list cannot carry them: that `create`/`clone` put the new
    /// session's id on stdout (so an agent can capture it and report it
    /// back, and knows the sentence it also sees is on stderr), and that
    /// `--host` takes a NAME out of the hosts listing rather than an
    /// identifier of some other kind. Losing either in a prose rewrite is
    /// the exact failure [`the_instructions_carry_the_conventions_nothing_
    /// else_teaches`] exists to catch for the older conventions.
    #[test]
    fn the_instructions_explain_what_the_creating_verbs_print_and_name() {
        let text = text();
        for needle in ["new session's id on stdout", "--host takes a name from"] {
            assert!(
                text.contains(needle),
                "the instructions no longer mention {needle:?}:\n{text}"
            );
