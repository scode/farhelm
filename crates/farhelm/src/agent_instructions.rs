...
/// carrying arguments — the shape every verb beyond `hosts`/`sessions`
/// will have, and one no current variant of [`AgentCmd`] can produce.
/// `agent` must already be built (see [`agent_command`]).
fn render(agent: &Command) -> String {
    ...
    out.push_str(
        "\n\
         You never have to pass a credential, a host, or a session id: this session's own\n\
         credential is already in your environment, so the lines above are complete command\n\
         ...
         \n\
         The answers come from the helm — the machine the user is sitting at — rather than\n\
         from this host, so they cover the whole fleet: every host and every session the helm\n\
...
mod tests {
    ...
    /// spelled differently.
    ///
    /// No current [`AgentCmd`] variant takes an argument, so without this
    /// the argument renderer would ship untested and would first be
    /// exercised by whichever future verb needs it — at which point a bug
    /// here shows up as an instruction line that tells an agent to run the
    /// wrong command line. The synthetic command stands in for the shape
    /// the mutating verbs (`rename`, `clone`) will have: one required
    /// positional, one optional value flag, one boolean flag.
    #[test]
