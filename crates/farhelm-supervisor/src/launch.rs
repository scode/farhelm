...
pub struct CheckoutPreparation {
    ...
    /// supervisor state, from [`preparation_state_path`]). Generated by the
    /// supervisor so both sides — the create path that publishes
    /// `NotStarted` and the shim that advances it — share one derivation.
...
/// Where one working copy's preparation state lives:
/// `<state_dir>/checkout-preparation/<working_copy_id>.json`.
///
/// Every supervisor path that publishes, reads, or removes that file derives
/// it here, and the shim receives the result rather than rebuilding it
/// ([`CheckoutPreparation::state_path`]), so the layout is stated once. The
/// depth is load-bearing: publishing fsyncs the state directory itself to make
/// the `checkout-preparation` entry durable, which is only the right parent
/// while that directory sits directly under the state directory.
///
/// The id is used as given. Callers that go on to unlink the file validate it
/// as a path component first (teardown does); readers get their check from
/// [`read_preparation_state`], which refuses a record naming another id.
pub fn preparation_state_path(state_dir: &Path, working_copy_id: &str) -> PathBuf {
    state_dir
        .join("checkout-preparation")
        .join(format!("{working_copy_id}.json"))
}
...
"#
    ...
    /// `<state_dir>/checkout-preparation/<id>.json`, and its lock sits beside
    /// it as `<id>.lock`.
    ///
    /// Why: publishing fsyncs the state directory on the assumption that the
    /// preparation directory sits directly under it, and every supervisor path
    /// that reads or removes these files goes through the helper. Pinned with
    /// literals so a change to the helper fails here rather than silently
    /// moving where all of them look.
    #[farhelm_testtrace::test]
    fn preparation_files_live_directly_under_the_state_dir() {
        let state_dir = Path::new("/state");
        let state = preparation_state_path(state_dir, "wc-1");
        assert_eq!(state, Path::new("/state/checkout-preparation/wc-1.json"));
        assert_eq!(
            preparation_lock_path(&state),
            Path::new("/state/checkout-preparation/wc-1.lock")
        );
    }
}
