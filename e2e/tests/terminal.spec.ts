 ...
 * starts with an empty input line and only the fake agent's own banner and
 * prompt in its buffer, so an assertion about "the path that was inserted"
 * cannot match one another test left behind.
 *
 * Reuses the shared suite's `createTabSession` factory: it already mints a
 * per-test working directory and registers it for cleanup, and an attachment
 * test wants exactly that.
 */
