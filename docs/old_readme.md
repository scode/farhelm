<!-- Parked here while the top-level README is rebuilt; the new README keeps only the warning. -->
...
[the agent hook injection page](../website/src/content/docs/docs/agents/agent-hook-injection.md) for what the hook does,
what it never does, and how to turn it off.
...
- Create a session: choose "new" in the session-count heading, pick the host, pick an agent profile — every helm offers
  read-only Built-in profiles for Claude Code, Codex, and Muse alongside the editable definitions you store, shared by
...
[run recorder](test-run-evidence.md), including real-tmux integration; doctests run separately with cargo. Read retained
output for runtime skip reasons from tests that need a systemd user manager. Browser tests use
