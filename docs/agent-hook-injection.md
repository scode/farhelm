...
| `/opt/bin/my-wrapper …`                                                                       | generic              | no             | no hook and no scan; restart replays the profile's resume invocation verbatim if it has one, and offers a fresh launch only if it does not                 |
| `env FOO=1 claude …`, `bash -c 'claude …'`                                                    | generic              | no             | same as the wrapper row                                                                                                                                    |
...
lands back on the fresh-launch offer. For the two generic rows, set the profile's agent kind explicitly if you want the
hook: farhelm then appends the flags to the wrapper's argv, which only helps if your wrapper forwards its arguments to
the real agent.
