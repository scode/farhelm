...
- **Dispatch under the attachments and lifecycle locks.** `A1-C4`, `A1-C2`. Medium, small to medium, medium risk; one
  unit per finding. The restricted create arm (handlers.rs:2858) claims the parent's lifecycle lock across a
  `ResolveProfile` round trip to the helm. A delete parked on a retained agent fence (handlers.rs:1166, :1187) holds one
  of eight process-wide admission permits for up to the 600 s retention while the reply that would free it is dispatched
  by the loop those permits park. Fix: resolve the profile before taking the parent claim, then claim and re-check the
  credential; claim the fence before the permit or bound the wait well under retention with a retry-safe Conflict.
