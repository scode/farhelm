...
release are refused rather than trusted, so re-run provisioning on hosts that were set up before it.

Written from the diff at curation-seeding time; the PR has no description. Verify the "re-run provisioning" advice
before it ships.
