...
- Agent-kind integrations live in the supervisor as a small trait (`AgentIntegration`; `AgentKind` is the wire enum
  ...
  **Codex attribution and exact-record validation.** The Unix accept loop captures the kernel peer PID and its process
  ...
  package-manager wrappers while refusing a nested native Codex. The reporter itself must spell the installed hook
  invocation (`<farhelm> internal hook …`, matched syntactically so an upgraded supervisor still accepts older hooks),
  and every other non-Codex link must be a narrow shell trampoline directly invoking it
  ...
  **Shared attribution framework and the five-step admission.** The ancestry walk above is shared mechanics, not Codex
  ...
  claim (one shared session-keyed `capture_locks` registry across every kind and capture path, never the lifecycle
  claim, at most a second of waiting on the report path) and a reload comparing kind, generation, and the complete prior
  binding; the mutation-free runtime and vendor-root proofs with repeat attribution around the evidence; the atomic
  ...
  reconciliation passes take the same claim and reload before mirroring, carrying the row's version beside the identity,
  so memory-derived offers apply the same gate as row-derived ones without a second lookup — and a rejected report never
  triggers a readiness withdrawal through them.

  **Ownership provenance and the offer gate.** Migration 19 adds `capture_ownership_version`
  (`INTEGER NOT NULL
  ...
  OMP row offers fresh-only until its first proven report. Migration 22 adds `goose_launch_program` the same way: the
...
- The `profiles` table is bounded on both axes — 128 stored profiles per helm, 8 KiB of caller-supplied text per profile
  — so the unpaginated catalog reply stays predictably bounded. The helm combines those stored rows with six
  release-owned Claude Code, Codex, and Muse built-ins in its read and resolution paths; built-ins are never seeded,
  persisted, or mutable. The response carries an authoritative `builtin` boolean, defaulting false for stored rows and
  old responses, so the UI need not derive source policy from an opaque ID. Existing stored starters remain ordinary
