...
`Archive` — added additively within the version, since it had not yet shipped when they landed — each routed and
recorded through the exact same `sessions.rs` functions (`route_session`, the client call, `record_session`) the REST
`/rename`/`/stop`/`/archive` routes use, so a refusal an agent reads is the identical sentence the UI would have shown.
...
- `farhelm agent rename <title> [--session <id>]`, `farhelm agent stop [--session <id>]`, and
  ...
  listing tables use, so a scripted caller gets exactly one line rather than a table with one row.
